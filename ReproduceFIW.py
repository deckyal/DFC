...
def validatePre(modelType=1,useBCE=True, fr=1, wh = 0, wvl = 0,wcl = 0, nmlz = 0,includeVal = True,dirName='models/',
    ...
    listCSVVal = [dirAdd+str(testFold)+'.txt']
    isTest = True
    ...
    dirImage = curDir+ 'data/images'+dir_add+'New/FIDs/'
    dirList =  ['landmarks'+dir_add+"New",'heatmaps'+dir_add+"New"]
    
    
    IDV = FamilyDataset(data_list = listCSVVal,transforms=data_transforms,dirImage = dirImage,withLandmark=True,withHeatmap = wh,useIT = True,multiLabel = True,
                        convertToOneHot = False,dirList = dirList,isTest = True,rl = rl,testFold = testFold)
    ...
    fileName+='-'+str(testFold)
    fileRName+='-'+str(testFold)
    ...
    for i, (x1,gt,htmp,lm,dta1) in enumerate(dataloaderVal) :
        ...
        with torch.no_grad(): 
            ...
            #preprocess label. 
            labelMapping = np.load(curDir+'cIndex-'+str(testFold)+'.npy')
...
test = True
if test : 
    validatePre(modelType=1,useBCE =True,fr =0, wh = 1,wcl = 1, nmlz = 1, wvl = 0,includeVal =True, dirName = 'src/model-val/')
    exit(0)